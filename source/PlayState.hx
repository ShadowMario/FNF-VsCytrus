...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'coach' | 'workout':
				curStage = 'fitness';
				defaultCamZoom = 0.85;
				var bg:BGSprite = new BGSprite('stagebackFit', -670, -400, 0.95, 0.95);
		...
		FlxG.sound.list.add(new FlxSound().loadEmbedded(Paths.music('WorkoutCutscene')));
	...
	function workoutCutscene():Void
	{
		inCutscene = true;
		dad.visible = false;
		camHUD.visible = false;
		var fakeDad:Character = new Character(DAD_X, DAD_Y, 'cytrus-cutscene');
		fakeDad = new Character(DAD_X, DAD_Y, 'cytrus-cutscene');
		fakeDad.x += fakeDad.positionArray[0];
		fakeDad.y += fakeDad.positionArray[1];
		add(fakeDad);
		FlxG.sound.playMusic(Paths.music('WorkoutCutscene'), 1, false);
		moveCamera(true);
		...
		new FlxTimer().start(1, function(tmr:FlxTimer) {
			FlxG.camera.shake(0.008, 2, function() {
				FlxG.camera.shake(0.02, 2, function() {
					FlxG.camera.shake(0.05, 4);
				});
			});
		});

		new FlxTimer().start(7, function(tmr:FlxTimer) {
			white = new FlxSprite(-500, -500).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.WHITE);
			white.scrollFactor.set();
			white.alpha = 0;
			FlxTween.tween(white, {alpha: 1}, 0.15, {ease: FlxEase.linear});
			add(white);
		});
		...
		new FlxTimer().start(10, function(tmr:FlxTimer)
		{
			...
			FlxTween.tween(white, {alpha: 0}, 1.5, {ease: FlxEase.linear,
				onComplete: function(twn:FlxTween) {
					remove(white);
				}
			});
